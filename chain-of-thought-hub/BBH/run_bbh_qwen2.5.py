...
# this is problematic! for example, the model gives the final summary after analysis "the best option is D",
# the the function only looks for "answer is", which is not found, then it simply loop over options and pick the one
# which shows up in the analysis, thus becomes A
def extract_ans(ans, mode):
    ...
    if "answer is " in ans:
        ans_line = ans.split("answer is ")
    elif "option is " in ans:
        ans_line = ans.split("option is ")

    # Expect to see 'answer is' or 'option is'. If not, return whole string
    if not ans_line or len(ans_line) == 1:
        pass
    else:
        ans = ans_line[-1].strip()
    ...
    if mode == 'multiple_choice':
        ...
        # from end of the text to find is more robust, because answers often show up in the end
...
class HSD():
    def __init__(self):
        ...
        #
        # self.num_samples = len(self.gsm8k_test['question']) // 5 if float(self.model_size[:-1]) > 3 else len(
        #     self.gsm8k_test['question'])
        #
    ...
    def __call__(self, tasks, mode):
        for task in tasks:
            ...
            print_first = True
            sd = f'test_Qwen2.5-{self.model_size}_{self.args.prompt}_{self.sd}_{task}.txt'
            with open(sd, 'w') as fd:
                ...
                for q_ in tqdm(task_data['examples']):
                    ...
                    # the cot prompt is distracting for answer extraction and causes error
...
def main(multiple_choice_tasks=MULTIPLE_CHOICE_TASKS, free_form_tasks=FREE_FORM_TASKS):
    hsd = HSD()
    run_multiple_choice = hsd.args.task == 'all' or hsd.args.task == 'multiple_choice'
    run_free_form = hsd.args.task == 'all' or hsd.args.task == 'free_form'
    if run_multiple_choice:
        hsd(multiple_choice_tasks, mode='multiple_choice')
    if run_free_form:
        hsd(free_form_tasks, mode='free_form')
    return 
