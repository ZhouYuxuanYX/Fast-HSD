...
def evaluate_posterior(
    ...
    if logits_processor is None:
        ...
        if min_p is not None:
            ...
            for i in range(1, candidates.shape[1]):
                ...
                for j in range(candidates.shape[0]):
                    if is_eq[j]:
                        ...
                        r = random.random()
                        px = gtp[xi]
                        p_max_i = float(gtp.max())
                        acp = 1.0 if float(px) >= p_max_i * min_p else 0.0
                        if r <= acp:
            ...
            for i in range(1, candidates.shape[1]):
                ...
                for j in range(candidates.shape[0]):
                    if is_eq[j]:
                        ...
                        r = random.random()
                        px = gtp[xi]
                        h = gtp.float().clamp(min=1e-10)
                        entropy_i = -(h * h.log()).sum()
                        eta_t = gtp.new_tensor(eta)
                        threshold_i = float(torch.min(eta_t, torch.sqrt(eta_t) * torch.exp(-entropy_i)))
                        acp = min(float(px) / threshold_i, 1.0) if threshold_i > 1e-9 else 1.0
                        if r <= acp:
                            accept_cand = torch.cat((accept_cand, x[None]), dim=0)
                            accept_length += 1
                            best_candidate = j
            ...
            # eagle_spd with lenience: lenience=1.0 -> standard SPD, lenience<1.0 -> relaxed (qx=lenience)
            accept_length = 1
            accept_cand = candidates[0][:1]
            best_candidate = 0
            adjustflag = False
