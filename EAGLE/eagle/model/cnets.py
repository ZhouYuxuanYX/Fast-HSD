...
class Model(nn.Module):
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        ...
        ind_logprob_full_list = [scores]  # individual log-probs per draft node (same indexing as scores_list)
        ...
        for i in range(depth):
            ...
            ind_logprob_full_list.append(topk_p.view(-1))  # individual log-probs for all top_k^2 nodes at this level
        ...
        ind_logprob_full = torch.cat(ind_logprob_full_list, dim=0)  # same size as scores_list_flat
        draft_probs_flat = torch.exp(ind_logprob_full[top_scores_index].float())  # (total_tokens,)
        draft_probs = torch.cat([
            torch.ones(1, device=draft_probs_flat.device, dtype=draft_probs_flat.dtype),
            draft_probs_flat
        ], dim=0)[None]  # (1, total_tokens+1): pos-0 is sample_token placeholder, rest are draft probs
