...
CONFIG_PATH="${CONFIG_PATH:-${SCRIPT_DIR}/config_aime.json}"
...
MAX_EXPERIMENTS="${MAX_EXPERIMENTS:-__FROM_CONFIG__}"
...
print(f"MAX_EXPERIMENTS_FROM_CONFIG={q(max_experiments)}")
...
if [[ "${MAX_EXPERIMENTS}" == "__FROM_CONFIG__" ]]; then
    MAX_EXPERIMENTS=${MAX_EXPERIMENTS_FROM_CONFIG}
fi

if [[ ${MAX_EXPERIMENTS} =~ ^[0-9]+$ ]] && [[ ${MAX_EXPERIMENTS} -gt 0 ]] && [[ ${TOTAL_TASKS} -gt ${MAX_EXPERIMENTS} ]]; then
    echo "Limiting tasks from ${TOTAL_TASKS} to ${MAX_EXPERIMENTS}"
    TOTAL_TASKS=${MAX_EXPERIMENTS}
elif [[ ! ${MAX_EXPERIMENTS} =~ ^[0-9]+$ ]]; then
    echo "[ERROR] MAX_EXPERIMENTS must be a non-negative integer or __FROM_CONFIG__, got: ${MAX_EXPERIMENTS}" >&2
    exit 1
fi
...
run_experiment() {
    ...
    local RUN_NAME="${DATASET}_${METHOD}_${PARAM_VALUE}_seed${SEED_VALUE}_${SHARD_LABEL}_k${K}_${RUNSTAMP}"
    ...
    echo "[$(date)] Starting Job ${JOB_ID} on GPUs ${GPU_SET} | Method: ${METHOD} | Value: ${PARAM_VALUE} | Seed: ${SEED_VALUE} | Samples: ${SAMPLE_START}-${SAMPLE_END} | k=${K}"
